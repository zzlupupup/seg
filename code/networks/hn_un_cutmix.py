...
class HN(nn.Module):
    ...
    def forward(self,x, threshold=None, mask=None):
        ...
        if mask is not None :
            ...
            labeled_bs = pred_l.shape[0] // 2

            pred_l_label = mask * pred_l[:labeled_bs] + (1 - mask) * pred_l[labeled_bs:]
            pred_l_unlabel = mask * pred_l[labeled_bs:] + (1 - mask) * pred_l[:labeled_bs]
            pred_l_ = torch.cat((pred_l_label, pred_l_unlabel), dim=0)

            pred_r_label = mask * pred_r[:labeled_bs] + (1 - mask) * pred_r[labeled_bs:]
            pred_r_unlabel = mask * pred_r[labeled_bs:] + (1 - mask) * pred_r[:labeled_bs]
            pred_r_ = torch.cat((pred_r_label, pred_r_unlabel), dim=0)     

            pred_fusion = self.fusion(pred_l_, pred_r_, threshold)

            return pred_fusion, pred_l_, pred_r_
