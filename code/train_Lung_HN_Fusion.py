...
if __name__ == "__main__":
    ...
    for epoch_num in tqdm(range(max_epoch), ncols=70):
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            iter_num = iter_num + 1
            logging.info(f'iteration{iter_num}: loss={loss.item():.4f} || fusion={sup_loss_fusion.item():.4f}, l={sup_loss_l.item():.4f}, r={sup_loss_r.item():.4f}, sup_loss={sup_loss.item():.4f} || l={unsup_loss_l.item():.4f}, r={unsup_loss_r.item():.4f}, unsup_loss={unsup_loss.item():.4f}, weight={unsup_weight:.4f}')
            if iter_num % 100 == 0:
